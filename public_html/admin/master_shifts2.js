...
function alter_hours(member, newhours) {
  ...
  if (parent != self) {
    parent.display_hours(member, hourslist[member]);
  }
  return true;
}

function print_time(num) {
  var min = Number((num-Math.floor(num))/60);
  if (!min) {
    min = '00';
  }
  while (min.length < 2) {
  ...
  return (Number(Math.floor(num))+Number(8)) + ':' + min;
}
...
function display_busy(busyday,vals,st,nd) {
  ...
  var am_free = true;
  //go through start to finish
  for (var ii = Math.floor(st); ii <= nd; ii++) {
    if ((vals.indexOf(busyday[ii],0) > -1) ^ !am_free) {
      if (busyday[ii] == 8 && ii == Math.floor(st) &&
          st == ii+Number(.5)) {
        continue;
      }
      if (busyday == 9 && ii == Math.floor(nd) &&
          nd == ii+Number(.5)) {
        continue;
      }
      var tempii = ii;
...
function assign_shift(member, workshift, addshift, silent) {
  if (!member || typeof(hourslist[member]) == 'undefined') {
  ...
  if (addshift) {
    ...
    if (typeof(busylist[member]) != 'undefined') {
      if (workshift[1] > 0) {
        var flr1 = Math.floor(temp[0]);
        var flr2 = Math.floor(temp[1]);
        if (flr1 != temp[0]) {
          if (busylist[member][workshift[1]-1][flr1] == 8) {
            busylist[member][workshift[1]-1][flr1] = 3;
          }
          else {
            busylist[member][workshift[1]-1][flr1] = 9;
          }
          flr1++;
        }
        if (flr2 != temp[1]) {
          if (busylist[member][workshift[1]-1][flr2] == 9) {
            busylist[member][workshift[1]-1][flr2] = 3;
          }
          else {
            busylist[member][workshift[1]-1][flr2] = 8;
          }
          flr2--;
        }
        for (var mpoi= flr1; ii <= flr2; ii++) {
  ...
  //dropping a shift?
    ...
    if (shiftlist[member][shift_string(workshift)]) {
      ...
      if (workshift[1] > 0) {
        ...
        var flr2 = Math.floor(temp[1]);
        if (flr1 != temp[0]) {
          if (busylist[member][workshift[1]-1][flr1] == 3) {
            busylist[member][workshift[1]-1][flr1] = 8;
          }
          else {
            busylist[member][workshift[1]-1][flr1] = origbusylist[member][workshift[1]-1][flr1];
          }
          temp[0] = flr1+Number(1);
        }
        if (flr2 != temp[1]) {
          if (busylist[member][workshift[1]-1][flr2] == 3) {
            busylist[member][workshift[1]-1][flr2] = 9;
          }
          else {
            busylist[member][workshift[1]-1][flr2] = origbusylist[member][workshift[1]-1][flr2];
          }
          temp[1] = flr2-1;
        }
...
function can_do(member,workshift,listing,silent) {
  ...
  if (hourslist[member] + Number(workshift[5]) > weekly_hours_quota) {
    if (listing) {
      return false;
    }
    else if (!silent) {
  ...
  if (hrmin[1] && hrmin[1].length) {
    hrmin[0] = Number(hrmin[0]) + Number(hrmin[1]/60);
  }
  ...
  if (nd <= st) {
    nd=Number(nd)+Number(24);
  }
  var hours = workshift[5];
  for (var tryct = 0; tryct < 2; tryct++) {
    ...
    //time up to 0
    if (st < 0) {
      var rnd = Math.floor(hours+st);
      for (var ii = 0; ii < hours+st; ii++) {
        if (busyday[ii] == 3 || busyday[ii] == 8 || (tryct == 0 && busyday[ii] == 2) ||
            (ii != flr && busyday[ii] == 9)) {
          break;
        }
      }
    ...
    if (nd > 16) {
      var rnd = Math.floor(nd-hours);
      for (var ii = 15; ii > nd-hours; ii--) {
        if (busyday[ii] == 3 || busyday[ii] == 9 || (tryct == 0 && busyday[ii] == 2) ||
            (ii != flr && busyday[ii] == 8)) {
          break;
        }
      }
    ...
    //hit the end time
    var rnd = Math.floor(st);
    var rnd2 = Math.floor(nd);
    var stoffset = st-rnd;
    var ndoffset = 0;
    var stii = -1;
    for (var ii = rnd; ii < nd; ii++) {
      ...
      if (busyday[ii] == 8 || (ii == rnd && (st-rnd))) {
        count = rnd-st;
        stoffset = -count;
        stii = ii;
      }  
      if (busyday[ii] == 9) {
        if (Number(count)+Number(.5) < hours) {
          count = 0;
          stoffset = 0;
          stii = -1;
          continue;
      ...
      if (++count >= hours) {
        ...
        return [stii+Number(stoffset),ii+Number(ndoffset)];
      }
    }
  }
  //did we fail?
