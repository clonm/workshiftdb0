...
if ($cur_week == -2) {
  exit("<h3>Your workshift manager has not set up when the semester starts yet.</h3>");
}
...
$redo_status = false;
$workshift_manager_email = get_static('workshift_manager_email',$house_name . 'wm1@usca.coop');
if (isset($_REQUEST['mem_name'])) {
  ...
  if (get_static('online_signoff_verifier_password',true) &&
      check_passwd($_REQUEST['verifier_name'],$_REQUEST['verifier_password']) <= 0) {
  ...
  if (array_key_exists('member_shift_submit',$_REQUEST)) {
    if (!isset($_REQUEST['mem_shift'])) {
      print "<h3>You did not choose a shift of your own for which to sign off!</h3>";
      break;
    }
    ...
    $row = $db->GetRow("select `hours`,`workshift` from `week_$signoff_weeknum` where " .
                       "`autoid` = ? and `member_name` = ? and " .
                       "`online_signoff` = 0 and " .
                       "date_add(date_add(`date`,interval ifnull(`end_time`,'01:00:00:00') day_second)," .
                       "interval ? hour) >= ?",
                       array($signoff_autoid,$_REQUEST['mem_name'],$max_signoff_time,
                             user_time(null,'Y-m-d H:i:s')));
    if (is_empty($row)) {
      print "<h3>Error!  " . escape_html($_REQUEST['mem_name']) . " cannot " .
        "sign off for the specified shift.  Please <a
href='" . escape_html($_SERVER['REQUEST_URI']) . "'>click here</a>, reload
the page, check your entries and" .
        "and try again.</h3>";
      break;
    }
    else if ($row['hours'] != $_REQUEST['mem_shift_hours']) {
      $db->Execute("update `week_$signoff_weeknum` set `hours` = ?," .
                   "`verifier` = ?,`notes` = concat(if(`notes`,concat(`notes`,' '),''),?),`online_signoff` = ? where `autoid` = ?",
      ...
      $db->Execute("update `week_$signoff_weeknum` set `verifier` = ?," .
                   "`notes` = concat(if(`notes`,concat(`notes`,' '),''),?),`online_signoff` = ? where `autoid` = ?",
    ...
    if (!is_numeric($_REQUEST['all_hours'])) {
      print "<h3>" . escape_html($_REQUEST['all_hours']) .
        " is not a valid number of hours for the shift you are signing off for.</h3>";
      break;
    }
    $row = $db->GetRow("select `hours`,`member_name` from `week_$signoff_weeknum` where " .
                       "`autoid` = ? and `workshift` = ? and strcmp(concat(`day`,' ',date_format(`date`,'%c/%e')), ?) = 0 and " .
                       "`online_signoff` = 0 and " .
                       "date_add(date_add(`date`,interval ifnull(`end_time`,'01:00:00:00') day_second)," .
                       "interval ? hour) >= ?",
                       array($signoff_autoid,$_REQUEST['all_shift'],$_REQUEST['day_name'],$max_signoff_time,
                             user_time(null,'Y-m-d H:i:s')));
    if (is_empty($row)) {
      print "<h3>Error!  The shift " . escape_html($_REQUEST['all_shift']) . " on " .
        escape_html($_REQUEST['day_name']) . " does not exist for online signoff.  " .
        "Please <a
href='" . escape_html($_SERVER['REQUEST_URI']) . "'>click here</a> and reload the page.</h3>";
      break;
    }
    else if ($row['hours'] != $_REQUEST['all_hours']) {
      $db->Execute("update `week_$signoff_weeknum` set `member_name` = ?, " .
                   "`hours` = ?,`verifier` = ?,`notes` = concat(if(`notes`,concat(`notes`,' '),''),?)," .
                   "`online_signoff` = ? where `autoid` = ?",
                   array($_REQUEST['mem_name'],$_REQUEST['all_hours'],
                         $_REQUEST['verifier_name'],'ONLINE: (usually ' . 
                         ($row['member_name'] == $_REQUEST['mem_name']?'':$row['member_name'] . ' ') .
                         $row['hours'] . " hrs) ",
                         user_time(null,'Y-m-d H:i:s'),$signoff_autoid));
    }
    else {
      $db->Execute("update `week_$signoff_weeknum` set `member_name` = ?, " .
                   "`verifier` = ?,`notes` = concat(if(`notes`,concat(`notes`,' '),''),?)," .
                   "`online_signoff` = ? where `autoid` = ?",
    ...
    $db->Execute("insert into `week_$cur_week` " .
  ...
  $redo_status = false;
  //  var_dump(get_static('online_signoff_email',null));
  //var_dump(rand(1,get_static('online_signoff_email',null)) == 1);
...
if ($cur_week) {
  $sql_more = "select " .
    ($cur_week-1) . " as `week`, `autoid`,concat(`day`,' ',date_format(`date`,'%c/%e')) as `dayte`,`workshift`,`member_name`,`hours` from `week_" . ($cur_week-1) . "` where " .
    "`online_signoff` = 0 and " .
    "date_add(date_add(`date`,interval ifnull(`end_time`,'01:00:00:00') day_second)," .
    "interval ? hour) >= ? union ";
...
$res = $db->Execute($sql_more . 
             "select $cur_week as `week`, `autoid`,concat(`day`,' ',date_format(`date`,'%c/%e')) as `dayte`,`workshift`,`member_name`,`hours` from `week_$cur_week` where " .
                    "`online_signoff` = 0 and " .
                    "date_add(date_add(`date`,interval ifnull(`end_time`,'01:00:00:00') day_second)," .
                    "interval ? hour) >= ?",
                    array_merge($arr,array($max_signoff_time,user_time(null,'Y-m-d H:i:s'))));
$member_shifts = array();
$day_shifts = array();
