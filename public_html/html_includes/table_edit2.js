...
function process_keypress(e) {
  ...
  //page-down
  if (code == 34 && key_modifier < 17 && targ.nodeName && targ.nodeName == 'INPUT' && targ.type && targ.type == 'text') {
    //spreadsheet page-down
    forward_row(targ,15,1);
    return false;
  }
  //page-up
  if (code == 33 && key_modifier < 17 && targ.nodeName && targ.nodeName == 'INPUT' && targ.type && targ.type == 'text') {
    //spreadsheet page-down
    forward_row(targ,-15,1);
    return false;
  }
  //downarrow
  if (code == 40 && key_modifier != 16 && 
      targ.nodeName && targ.nodeName == 'INPUT' && targ.type && targ.type == 'text') {
...
function default_change_handler (elt) {
  ...
  if (is_input(elt) && !is_checkbox(elt)) {
    ...
    var style_ind = theRules.length-num_cols+Number(elts[1]);
...
//   }
  ...
  for (ii = 0; ii < num_cols; ii++) {
    data += "col_names[]=" + val_of(cols[ii].firstChild) + "&";
  }
